...
const IndexingAlert = ({ className }: { className?: string }) => {
  ...
  React.useEffect(() => {
    if (!isLoading && !isError) {
      cookies.set(
        ...
        data.finished_indexing && data.finished_indexing_blocks
          ? "false"
          : "true"
      );
    }
  ...
  const handleInternalTxsIndexStatus: SocketMessage.InternalTxsIndexStatus["handler"] =
    React.useCallback(
      (payload) => {
        queryClient.setQueryData(
          getResourceKey("homepage_indexing_status"),
  ...
  let content;
  if (data.finished_indexing_blocks === true) {
    content = `${
  ...
  } else if (data.finished_indexing === true) {
    content = `${
      data.indexed_internal_transactions_ratio &&
      `${Math.floor(
        Number(data.indexed_internal_transactions_ratio) * 100
      )}% Blocks With Internal Transactions Indexed${nbsp}${ndash} `
    }
          We're indexing this chain right now. Some of the counts may be inaccurate.`;
  }
